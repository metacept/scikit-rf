...
# THRU_85521A = base_ntwk.line(90,unit ='deg')

THRU_85521A.z0 = (50+0j)*np.ones((n_freq, 4)) 

fig, ax = plt.subplots()
THRU_85521A.plot_s_deg(axis =ax, linewidth= 2)
THRU_85521A.plot_s_deg(axis =ax, linestyle='--', linewidth= 2)
...
cal_folder_name = '2021-04-30_SOLT'
...
#%%
def take_measurement_return_ntwk():
    vna.sweep
# S11
...
#     # save results
#     folder = Path('/Users/Zaber/Documents/data/scikit_measurements/Mike_Metasurface_3/measurements/')
#     file = folder / dut_corrected.name
#     dut_corrected.write_touchstone(str(file))
 #%%
# fig, ax = plt.subplots()
...
def apply_cal_to_measurement_and_save(dut_raw, name, folder):
    
    dut_raw.name = name
    dut_corrected = cal_TwelveTerm.apply_cal(dut_raw)
    dut_corrected.name =  dut_raw.name + '_corrected'
    ...
    # save results
    file = folder / dut_raw.name
    file_corrected = folder / dut_corrected.name
    
    dut_raw.write_touchstone(str(file))
    dut_corrected.write_touchstone(str(file_corrected))
    
    return dut_corrected, ax1, ax2
#%%
folder = Path('/Users/Zaber/Documents/data/scikit_measurements/Mike_Metasurface_3/measurements/')

measurement = take_measurement_return_ntwk()
dut, ax1, ax2= apply_cal_to_measurement_and_save(measurement, 'channel_dummy', folder)
...
dut.plot_s_deg(ax = ax1)
dut.plot_s_db(ax = ax2)
